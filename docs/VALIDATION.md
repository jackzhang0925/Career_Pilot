...
| Launch from GitHub | Configured; production execution requires repository secrets | `Actions → Launch → Run workflow` and pushes to `main` validate then deploy via the protected `production` environment. Cloudflare deployment packaging passes a local dry run. |
...
1. Add `CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ACCOUNT_ID` to the GitHub `production` Environment, then run the Launch workflow once.
