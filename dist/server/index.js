globalThis.__VINEXT_LAZY_CHUNKS__ = ["assets/career-dashboard-pLyF3X6o.js","assets/lib-B3X-f5Gv.js","assets/pdf-DrwCno2x.js","assets/layout-segment-context-BGMeYrBA.js"];
...
function getDraftSecret() {
	return "01e496d8-f6f9-44df-bd9c-c97630f28af3";
}
/**
...
var NextURL = class NextURL {
	...
	*/
	get buildId() {
		return "ef9e2806-a25b-47e5-9e7a-79664c466228";
	}
};
...
*/
function appIsrCacheKey(pathname, suffix, buildId = "ef9e2806-a25b-47e5-9e7a-79664c466228") {
...
function createAppPageArtifactCompatibility(element, routePattern) {
	...
	return createArtifactCompatibilityEnvelope({
		...
		deploymentVersion: "ef9e2806-a25b-47e5-9e7a-79664c466228",
		rootBoundaryId
	});
}
/**
...
var CareerDashboard = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'CareerDashboard' is called on server");
}, "29b5d3f650f6", "CareerDashboard");
//#endregion
//#region app/page.tsx
...
function Home() {
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(CareerDashboard, {});
}
...
//#endregion
//#region app/server/openai-key-store.ts
var runtime = globalThis;
function getRuntimeOpenAISettings() {
	return runtime.__catCareerOpenAI;
}
function setRuntimeOpenAISettings(settings) {
	runtime.__catCareerOpenAI = settings;
}
function clearRuntimeOpenAISettings() {
	delete runtime.__catCareerOpenAI;
}
//#endregion
//#region app/api/analyze-resume/route.ts
...
async function GET$1() {
	...
	return Response.json({
		configured: Boolean(runtime?.apiKey || process.env.OPENAI_API_KEY),
		model: runtime?.model || process.env.OPENAI_MODEL || "gpt-5.6-sol"
	});
}
async function POST$3(request) {
	...
	const response = await fetch("https://api.openai.com/v1/responses", {
		...
		body: JSON.stringify({
			...
			input: [{
				...
				role: "user",
				content: `请分析下面的简历并生成求职画像：\n\n${resumeText.slice(0, 3e4)}`
			}],
			text: { format: {
				type: "json_schema",
				name: "career_profile",
	...
	if (!response.ok) {
		const message = payload && typeof payload === "object" && "error" in payload ? payload.error?.message : void 0;
		return Response.json({ error: message || "AI 分析暂时不可用。" }, { status: response.status });
	}
	try {
		return Response.json(JSON.parse(outputText$1(payload)));
	} catch {
		return Response.json({ error: "AI 返回内容无法解析。" }, { status: 502 });
	}
}
//#endregion
//#region app/api/career-coach/route.ts
...
async function POST$2(request) {
	...
	const history = (body?.messages || []).slice(-10).flatMap((message) => {
		...
		const content = message.content?.trim().slice(0, 4e3);
		return role && content ? [{
			role,
			content
		}] : [];
...
//#endregion
//#region app/api/openai-key/route.ts
...
async function POST$1(request) {
	if (!isLocalRequest(request)) return Response.json({
		valid: false,
		error: "临时 API Key 只能在本机 localhost 使用。"
	}, { status: 403 });
	const body = await request.json().catch(() => null);
	...
	if (!response) return Response.json({
		valid: false,
		error: "无法连接 OpenAI，请检查网络后重试。"
	}, { status: 502 });
	if (!response.ok) {
		const payload = await response.json().catch(() => null);
		return Response.json({
			valid: false,
			error: payload?.error?.message || "API Key 或模型不可用。"
		}, { status: response.status });
	}
	setRuntimeOpenAISettings({
		apiKey,
		model
	});
	return Response.json({
...
async function DELETE(request) {
	if (!isLocalRequest(request)) return Response.json({
		removed: false,
		error: "临时 API Key 只能在本机 localhost 移除。"
	}, { status: 403 });
...
//#endregion
//#region app/api/scan-jobs/route.ts
var route_exports = /* @__PURE__ */ __exportAll({ POST: () => POST });
var greenhouseBoards = [
	{
		company: "Stripe",
		board: "stripe",
		color: "#635bff"
...
var companyColors = new Map(greenhouseBoards.map((source) => [source.company, source.color]));
...
function scoreCandidate(candidate, profile) {
	...
	const locationMatch = [
		...
		"canada",
		"remote"
	].some((place) => preferredLocation.includes(place) && locationText.includes(place));
	let score = 38;
	...
	const reason = exactRole ? `职位名称与目标画像直接重合${locationMatch ? "，地点也符合偏好" : ""}` : sameFamily ? `属于同一目标职能，重合关键词：${overlap.slice(0, 3).join("、") || "职责方向"}` : overlap.length ? `部分经历可迁移，重合关键词：${overlap.slice(0, 3).join("、")}` : "与当前画像关联较弱，仅保留在职位池供你复核";
	return {
		score,
		reason,
...
function relativeDate(value) {
	if (!value) return "近期";
	const days = Math.max(0, Math.floor((Date.now() - new Date(value).getTime()) / 864e5));
	if (days === 0) return "今天";
	if (days === 1) return "1 天前";
	if (days < 30) return `${days} 天前`;
	return "30 天内";
}
async function fetchGreenhouse(company, board) {
	const response = await fetch(`https://boards-api.greenhouse.io/v1/boards/${board}/jobs?content=true`, { signal: AbortSignal.timeout(12e3) });
	if (!response.ok) throw new Error(`${company} Greenhouse returned ${response.status}`);
	return ((await response.json()).jobs || []).map((job) => ({
...
async function fetchLever(company, board) {
	const response = await fetch(`https://api.lever.co/v0/postings/${board}?mode=json`, { signal: AbortSignal.timeout(12e3) });
	if (!response.ok) throw new Error(`${company} Lever returned ${response.status}`);
	return (await response.json()).filter((job) => job.hostedUrl).map((job) => ({
		externalId: `${board}-${job.id}`,
		company,
		role: job.text,
		...
		source: "Lever",
		url: job.hostedUrl,
		publishedAt: job.createdAt ? new Date(job.createdAt).toISOString() : void 0,
		description: [
...
async function fetchCohere() {
	const response = await fetch("https://api.ashbyhq.com/posting-api/job-board/cohere", { signal: AbortSignal.timeout(12e3) });
	if (!response.ok) throw new Error(`Cohere Ashby returned ${response.status}`);
	return ((await response.json()).jobs || []).filter((job) => job.jobUrl).map((job) => ({
...
async function POST(request) {
	...
	const requested = Array.isArray(profile.sources) ? profile.sources : [
		"Greenhouse",
		"Lever",
		"Ashby"
	];
	const requestedSources = new Set(requested.filter((source) => [
		"Greenhouse",
		"Lever",
		"Ashby"
	].includes(source)));
	const fetchers = [
		...greenhouseBoards.filter(() => requestedSources.has("Greenhouse")).map((source) => ({
			name: `${source.company} · Greenhouse`,
			run: () => fetchGreenhouse(source.company, source.board)
		})),
		...leverBoards.filter(() => requestedSources.has("Lever")).map((source) => ({
			name: `${source.company} · Lever`,
			run: () => fetchLever(source.company, source.board)
		})),
		...requestedSources.has("Ashby") ? [{
			name: "Cohere · Ashby",
			run: fetchCohere
		}] : []
	];
	if (!fetchers.length) return Response.json({ error: "请至少启用一个可自动扫描的公开职位来源。" }, {
		status: 400,
	...
	if (succeeded === 0) return Response.json({
		error: "所有职位来源暂时不可用。",
		failures,
	...
	return Response.json({
		jobs: ranked.slice(0, 38),
		scanned: deduplicated.length,
		sources: succeeded,
		attemptedSources: fetchers.length,
		failures,
		fetchedAt: (/* @__PURE__ */ new Date()).toISOString()
	}, { headers: { "Cache-Control": "no-store" } });
}
...
var routes = [
	...
	},
	{
		...
		ids: {
			"route": "route:/api/analyze-resume",
			"page": null,
			"routeHandler": "route-handler:/api/analyze-resume",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/analyze-resume",
		patternParts: ["api", "analyze-resume"],
		isDynamic: false,
		params: [],
		rootParamNames: [],
		page: null,
		...
		layouts: [layout_exports],
		routeSegments: ["api", "analyze-resume"],
		templateTreePositions: [],
		layoutTreePositions: [0],
		templates: [],
		errors: [null],
	...
	},
	{
		...
		ids: {
			"route": "route:/api/career-coach",
			"page": null,
			"routeHandler": "route-handler:/api/career-coach",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/career-coach",
		patternParts: ["api", "career-coach"],
		isDynamic: false,
		params: [],
		rootParamNames: [],
		page: null,
		...
		layouts: [layout_exports],
		routeSegments: ["api", "career-coach"],
		templateTreePositions: [],
		layoutTreePositions: [0],
		templates: [],
		errors: [null],
	...
	},
	{
		...
		ids: {
			"route": "route:/api/openai-key",
			"page": null,
			"routeHandler": "route-handler:/api/openai-key",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/openai-key",
		patternParts: ["api", "openai-key"],
		isDynamic: false,
		params: [],
		rootParamNames: [],
		page: null,
	...
	},
	{
		...
		ids: {
			"route": "route:/api/scan-jobs",
			"page": null,
			"routeHandler": "route-handler:/api/scan-jobs",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/scan-jobs",
		patternParts: ["api", "scan-jobs"],
		isDynamic: false,
		params: [],
		rootParamNames: [],
		page: null,
